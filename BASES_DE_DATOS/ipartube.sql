...
INSERT INTO `videos` VALUES (1,'Michael Jackson en Auckly','Con efectos 3D de la época.\r\nComo se nota la diferencia...','2026-01-22 10:26:00','https://i.ytimg.com/vi/ChrLRauOR28/hq720.jpg','https://www.youtube.com/embed/ChrLRauOR28',1),(3,'Clásicos del Rock','',NULL,'https://i.ytimg.com/vi/4_O-y3SM3UM/hq720.jpg','https://www.youtube.com/embed/4_O-y3SM3UM',2),(4,'8BBB en directo','Video de la 8 Bit Bit Band en diciembre de 2025','2026-01-27 11:03:00','https://i.ytimg.com/vi/mzjqdhWxg8M/hq720.jpg','https://www.youtube.com/embed/mzjqdhWxg8M',2),(5,'Prueba',NULL,NULL,NULL,'https://alskdfjlkajsdf',1);
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `videos_completos`(_usuario BIGINT)
BEGIN
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `usuarios_completos` AS select `u`.`id` AS `u_id`,`u`.`nombre` AS `u_nombre`,`u`.`email` AS `u_email`,`u`.`imagen_url` AS `u_imagen_url`,`r`.`id` AS `r_id`,`r`.`nombre` AS `r_nombre`,`r`.`descripcion` AS `r_descripcion` from (`usuarios` `u` join `roles` `r` on((`u`.`roles_id` = `r`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-02-18 11:04:27
