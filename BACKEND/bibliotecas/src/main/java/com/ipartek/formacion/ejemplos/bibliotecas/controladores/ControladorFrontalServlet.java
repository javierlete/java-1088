...
public class ControladorFrontalServlet extends HttpServlet {
	...
	public ControladorFrontalServlet() {
		...
		if (paqueteRutas == null) {
			throw new ControladorFrontalException(
					"No se ha encontrado la ruta de paquetes en el configuracion.properties cf.paqueterutas");
		}
	}
	...
	private String ejecutarAccion(String ruta, Map<String, String[]> entrada, Map<String, Object> salida,
		...
		try (ScanResult scan = new ClassGraph().enableAllInfo().acceptPackages(paqueteRutas)
			...
			throw new ControladorFrontalException("No se ha podido cargar la ruta " + ruta, e);
		}

		throw new ControladorFrontalException("No se ha encontrado la @Ruta");
	}
